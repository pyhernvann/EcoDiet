...
check_numeric_parameter <- function(numeric_parameter, parameter_name){
  ...
  if (numeric_parameter < 0){
    stop("You can only enter a null or positive number for the \"", parameter_name, "\" parameter.\n",
         "  Please put a positive value instead.")
  }
...
#' @param nb_eta_prior the equivalent number of stomach for the literature priors on eta
#' @param nb_PI_prior the equivalent number of isotope for the literature priors on PI
#' @param nb_eta_to_PI the equivalent number of stomach for which eta will impact PI
#' 
#' @return a list of preprocessed data, ready to be run by the EcoDiet model
#'
#' @export

preprocess_data2 <- function(isotope_data, trophic_enrichment_factor,
  ...
  check_numeric_parameter(nb_eta_to_PI, "nb_eta_to_PI")
  ...
  data_list <- list(
    ...
    ID         = diag(nb_elem),
    nb_eta_to_PI     = nb_eta_to_PI
  )
    
  if (literature_prior){
    
    check_numeric_parameter(nb_eta_prior, "nb_eta_prior")
    check_numeric_parameter(nb_PI_prior,  "nb_PI_prior")
    
    # Compute the zdeta parameter from the literature pedigree and the literature number
    dzeta <- nb_eta_prior * literature_pedigrees + 1
