...
In this vignette, we will explain the later and simpler use. If you want to use the EcoDiet model with data from the literature, please read the `literature_prior` vignette.
...
example_stomach_data_path <- system.file("extdata", "example_stomach_data.csv",
                                    package = "EcoDiet")
knitr::kable(read.csv(example_stomach_data_path))
```
...
example_isotope_data_path <- system.file("extdata", "example_isotope_data.csv",
                                    package = "EcoDiet")
knitr::kable(read.csv(example_isotope_data_path))
```

Each line representes one isotope sample, the first column indicates to which group the individual belongs, the second column indicates the sample index (it should go from 1 to the total number of samples for this group), and the other columns contain the isotopic measures.

## Load the data

To load your data, you will need to know the path of the csv files in your computer. Here are the paths for the example data: 
```{r}
...
The `preprocess_data` function loads the data and rearrange it in a specific format for the EcoDiet model to be able to run. You need to put **your own data paths**:
```{r}
...
Note that you need to specify the trophic enrichment factors that **corresponds** to your isotopic data! If your isotopic data files contains d13C and d15N data as in the example, then you should enter the trophic enrichment factor for the d13C and d15N isotopes (in that order).
