...
vignette: >
  %\VignetteIndexEntry{Introduction - How to use EcoDiet}
...
* *Literature option* - Stomach content and biotracers data are integrated and results from a literature search are used to formulate informative priors. 

We will explain the two possibilities in this vignette.
...
Note that the example we will use in this vignette is a toy dataset, created to be simple to visualize and understand.
...
You will also need to define the trophic enrichment factors that corresponds to your isotopic data. If your isotopic data files contains d13C and d15N data as in the example, then you should enter the trophic enrichment factors for the d13C and d15N isotopes (in that order):
```{r, eval = FALSE}
...
## Define your prior (*Literature option*)

**Skip this part if your want to use EcoDiet without prior from the literature.**

The calculation of the priors for the variables "eta" and "PI" from the literature values and the Pedigree scrore also involves to import a litterature diet matrix, and two additional parameters allowing to modulate the relationship between the Pedigree value and the priors' precision.
...
## Preprocess the data and prior (*Literature option*)
...
The matrix displayed by the `preprocess_data` function is based:
* on the stomach content data, so by-default, EcoDiet will investigate trophic links only based on what you have found in stomachs analyses, and
* on the literature diets matrix if you have defined priors, so by-default, EcoDiet will also investigate trophic links only if you have identified them as plausible regarding your literature search.

If you want to add another trophic link, you can modify directly the topological matrix. The by-default topological matrix can be defined by the following instruction : 
...
If you find the plot to contain too much information, you can also precise for which prey you want to see the probability density:
...
You can now plot the prior distributions you are interested in:
