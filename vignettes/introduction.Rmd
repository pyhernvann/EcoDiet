...
Note that biotracers data will be here illustrated by stable isotope only, but EcoDiet could be used to treat other analyses such as fatty acids or specific-compounds stable isotopes.
...
You need to load the data you want to be analyzed by EcoDiet. You can load your data by importing it from `.csv` files. To be treated by the package, your data should be in specific formats, similar to those of following example.
...
The last row of the table indicates how many (non-empty) stomachs have been analyzed for each trophic group. In this example, for the "huge" animals, 19 stomachs were analyzed and contained remainings. Among these stomachs, 17 contain "Large" animal remainings and 12 contain "medium" animal remainings.

The example is of course an artificial dataset, created to be simple to visualize and understand.
