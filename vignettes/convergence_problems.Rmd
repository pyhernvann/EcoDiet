...
* The 'convergence problem' warning means that the Gelman-Rubin diagnostic stayed above 1.1 for at least one variable. The Gelman-Rubin statistic is used to measure convergence for each variable.
...
So you should try to increase the `nb_iter` argument, althouh this will likely takes several hours/days to run:
```{r, eval = FALSE}
mcmc_output <- run_model(textConnection(model_string), data, nb_iter = 1e+07)
```
...
If the `PI` and `eta` variables has both trouble converging for a specific trophic link between a predator and a prey, you can look at the corresponding data to see if there is not an error somewhere (maybe an isotopic measurement was entered as 104 instead of 1.04 for this prey or predator).
...
First try to change the prior distributions used through the `literature-diets` table, the `nb_literature` and the `literature_slope` parameters to make the literature priors less inconsistent with the data (if you used priors from the literature).
