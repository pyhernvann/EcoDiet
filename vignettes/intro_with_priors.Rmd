...
1. Technically, proportions proposed a priori are used as the parameters "alpha" of the Dirichlet distributions used as prior for the diet proportions. By default (no a priori information), the Dirichlet prior is uninforamtive with parameter alpha = 1/k where k is the number of trophic links identified from the stomach content data.  
...
All numbers will then be used to set the parameters for the prior Beta distributions of the probabilities of a trophic link. For each number > 0, the probabilities will be shiflted toward 1, while for each number = 0, the probabilities will be shifted toward 0.
...
### Weighting the influence of prior relative to the data

The weight of the informative prior can be modulated using three additionnal options that can be modified by the user (et sinon, y'a t'il des options par défaut ed prévues " ?: 
- Pedigree
- Nb Literature
- Literature slope
