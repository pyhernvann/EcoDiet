...
The goal of EcoDiet is to combine biotracers and stomach content analyses together with *a priori* knowledge from the literature to simulataneously identify who eats whome in the food web (**topology matrix**), and in which proportions (**diet matrix**). More specifically, EcoDiet is an integrated Bayesian model that uses *(i)* stomach content analyses and priors from studies found in the literature to estimate the probability that any trophic link exists *(ii)* biotracers analyses and priors from studies found in the literature to estimate the dietary proportions of each consumer in the food web conditionnally to the trophic links identified.
...
For a given consumer, the ratio between the product `nb_literature`*`literature_pedigrees` and the number of stomachs determines the influence of the literature relative to data. Thus, the higher `nb_literature` is the stronger will be the weight of the literature in the final infrerences. In practice, due to the inherent emptiness of stomachs when analyzing them, `nb_literature` should be set at a lower value in order to not giving too much weight to the literature. By default, it is set to 10 but it is case-specific and there are no rules for attributing a value to `nb_literature`. A good start can for instance be the mean number of stomachs analyzed per species.
...
## Plot the full results (the probability densities)

You can also plot the full results by visualizing the probability densities for the `PI` and `eta` variables (each different figure corresponds to one predator):
