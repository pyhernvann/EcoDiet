...
The goal of the package is to estimate a probabilistic topology matrix (all trophic link probabilities $\eta$) and a diet matrix (all diet propotions $\Pi$) by combining biotracers and stomach content analyses in a Bayesian hierarchical model. A trophic link probability is the probability that a given prey in eaten by a given predator, and a diet proportion is the percentage of contribution of this prey to this predator's diet. The full model and its application on a real dataset are described in *Hernvann et al.* (under review).
...
If you want to add another trophic link, you can modify directly the topology matrix. It can be useful if you are sure that a prey is consumed by a given predator. However your stomach sampling size is too small to allow any observation. To specify that the "huge" animal can also eat "small" animals, you can do: 
...
Now the new topology matrix can be entered as an argument of the `preprocess_data` function, and you can see that the new trophic links matrix will be investigated by the model:
```{r}
data <- preprocess_data(biotracer_data = example_biotracer_data,
                        trophic_discrimination_factor = c(0.8, 3.4),
                        literature_configuration = literature_configuration,
...
The $\Pi$ distributions are very peaked around 0 and 1 because of how the variables are modelled in EcoDiet. It is a model with probabilistic trophic links, so:

* The model first evaluates whether the trophic link exists or not between a predator and his prey (based on the $\eta$ variables).
